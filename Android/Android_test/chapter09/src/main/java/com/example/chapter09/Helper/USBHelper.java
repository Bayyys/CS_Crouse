...
public class USBHelper {
    ...
    private USBHelper(Context _context) {
        intent = PendingIntent.getBroadcast(_context, 0, new Intent(ACTION_USB_PERMISSION), FLAG_IMMUTABLE);
    ...
    public void sendData(byte[] buffer) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true) {
                    byte[] buffer2 = new byte[epBulkIn.getMaxPacketSize()];
                    int length2 = conn.bulkTransfer(epBulkIn, buffer2, buffer2.length, 0);
                    if (length2 >= 0) {
                        Log.i(TAG, "接收成功" + length2 + "数据" + new String(buffer2, StandardCharsets.UTF_8));
        ...
        int length = conn.bulkTransfer(epBulkOut, buffer, buffer.length, 0);
        if (length >= 0) {
            //0 或者正数表示成功
            Log.i(TAG, "发送成功" + length + "数据" + new String(buffer));
            statue = USBInterface.usb_permission_ok;
        } else {
            Log.i(TAG, "发送失败");
            statue = USBInterface.usb_permission_fail;
