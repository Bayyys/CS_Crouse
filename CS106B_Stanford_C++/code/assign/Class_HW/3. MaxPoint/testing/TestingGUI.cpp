...
namespace SimpleTest {
   ...
   Vector<TestGroup> prepareGroups(const map<TestKey, multimap<int, TestCase>> &allTests)
   {
    ...
    {
        cout << endl;
        cout << "测试案例根据文件名或类型分组。" << endl;
        cout << "选择你想要执行的测试分组序号：" << endl;
        cout << "----------------------------------------" << endl;
        int i = 0;
        for (const auto& entry: options) {
            cout << "  " << i++ << ") " << entry << endl;
        }
        int chosen = getIntegerBetween("输入你的选择：", 0, options.size() - 1);
        if (chosen == 0) {
            cout << "未选择任何测试。" << endl << endl;
        }
        return chosen;
    }
    ...
    int getChoice(Vector<TestGroup>& groups, Choice ch, std::string groupName)
    {
        if (ch == ALL_TESTS ) {
            ...
            Vector<string> names = {"不测试，直接运行 main() 函数", "测试所有程序"};
            for (const auto& entry: groups) {
                int index = names.size() - 1;
                names.insert(index, "测试 " + entry.name);
